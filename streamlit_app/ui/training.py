...
def render_training_controls(
    ...
    if ready:
        ...
        is_terminal = display_state.get("is_terminal", st.session_state.get(is_terminal_key, True))
        if is_terminal and display_state["total_episodes"] > 0:
            st.success(
                f"🎉 Goal Reached! Episode {display_state['total_episodes']} complete."
            )
        
        if st.button("Train a new episode step by step", key=f"{tab_id}_new_episode", disabled=in_playback):
