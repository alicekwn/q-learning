...
from streamlit_app.state import rewind_checkpoint, forward_checkpoint, jump_to_latest, jump_to_start
...
def q_learning_parameters(tab_id: str) -> dict:
    ...
    return {"alpha": alpha, "gamma": gamma, "epsilon": epsilon, "reward_val": reward_val}
...
def playback_controls(config: dict, in_playback: bool) -> None:
    ...
    no_actions_taken = len(checkpoints) <= 1  # Only initial checkpoint exists (no actions yet)
    ...
    with col_prev:
        if st.button("◀️ Previous action", key=f"{tab_id}_rewind", disabled=prev_disabled):
            rewind_checkpoint(config)
            st.rerun()
    
    with col_next:
        if st.button("▶️ Next action", key=f"{tab_id}_forward", disabled=not in_playback):
            forward_checkpoint(config)
            st.rerun()
    
    with col_latest:
        if st.button("⏭️ Latest action", key=f"{tab_id}_latest", disabled=not in_playback):
