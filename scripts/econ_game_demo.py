"""
This script demonstrates the Q-learning algorithm applied to a simple economic game.
The game is a classic Bertrand duopoly with homogeneous products and linear demand.
The script trains Q-tables for two firms, allowing them to learn optimal pricing strategies
...
price_start = 4
price_end = 10
num_points = 7  # number of points in the interval
PRICES = np.linspace(price_start, price_end, num_points).tolist()
...
def demand1(p1, p2):  # q1
    return max(0.0, 7 - p1 + 0.5 * p2)


def demand2(p1, p2):  # q2
    return max(0.0, 7 - p2 + 0.5 * p1)


def profit1(p1, p2, marginal_cost=2.0):
    return (p1 - marginal_cost) * demand1(p1, p2)


def profit2(p1, p2, marginal_cost=2.0):
    return (p2 - marginal_cost) * demand2(p1, p2)
...
def train_episode(
    ...
    for t in range(1, max_periods + 1):  # loop over periods/steps
        ...
        pi1 = profit1(p1_next, p2_next, marginal_cost=marginal_cost)
        pi2 = profit2(p1_next, p2_next, marginal_cost=marginal_cost)
...
def get_cycle_stats(Q1, Q2, start_p1, start_p2, marginal_cost=2.0, max_steps=100000):
    """
    ...
    p1_profits = np.array(
        [
            profit1(p1, p2, marginal_cost=marginal_cost)
            for p1, p2 in zip(p1_prices, p2_prices)
        ],
        dtype=float,
    )
    p2_profits = np.array(
        [
            profit2(p1, p2, marginal_cost=marginal_cost)
            for p1, p2 in zip(p1_prices, p2_prices)
        ],
        dtype=float,
    )
...
pd.DataFrame(Q1, index=states, columns=actions).to_csv("Q1_demo.csv")
pd.DataFrame(Q2, index=states, columns=actions).to_csv("Q2_demo.csv")

print("\n--- Greedy trajectory starting at nash price ---")
traj_nash = follow_greedy_until_loop(
    Q1, Q2, nash_price, nash_price, max_steps=50000, verbose=True
)
...
print("\n--- Greedy trajectory starting at collusion price ---")
traj_collusion = follow_greedy_until_loop(
    Q1, Q2, collusion_price, collusion_price, max_steps=50000, verbose=True
)

print("\n--- Cycle stats starting at collusion price ---")
stats_collusion = get_cycle_stats(
    Q1,
    Q2,
